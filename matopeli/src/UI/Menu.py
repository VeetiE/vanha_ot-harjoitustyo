...
class Menu:
    def __init__(self) -> None:
        ...
        self.highscore = self.db.execute(
            'SELECT score FROM Users ORDER BY score DESC LIMIT 1;').fetchall()
        self.date = self.db.execute(
            'SELECT day, month, year FROM Users ORDER BY score DESC LIMIT 1').fetchall()
        ...
        # High Score Text
        self.high = self.font.render(
            'High Score: ' + f'{self.highscore[0][0]}', False, (255, 0, 255))
        ...
        self.date = self.font.render(
            f'Date: {self.date[0][0]}/{self.date[0][1]}/{self.date[0][2]}', False, (255, 0, 255))
        self.dateRect = self.high.get_rect()
        self.dateRect.center = (600//2, 600-200)

        # Start Text
        self.start = self.font.render('Press Space to Start', False, ((
